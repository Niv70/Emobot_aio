...
async def answer_tst(message: Message, state: FSMContext):
    ...
    if answer == "я чувствую интерес":
        await message.answer("У тебя здорово получается!\nОдин раз в день я буду давать тебе интересную ”задачку на про"
                             "качку” эмоционального интеллекта. Постарайся уделить время для ее выполнения  - это ОЧЕНЬ"
                             " ВАЖНО!", reply_markup=choice05)
