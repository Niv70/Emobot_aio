...
async def answer_12_01(message: Message, state: FSMContext):
    ...
    if s == "Выполнить сейчас!":
        await message.answer("Cсылка на видео-фрагмент из фильма “Служебный роман”:\n"
                             "https://www.youtube.com/watch?v=q8CJ9cVRh3c")
        await message.answer("Посмотри, как может меняться эмоция в зависимости от походки (обрати внимание, как в конц"
                             "е меняется положение тела Людмилы Прокопьевны при появлении подчиненных).\nНапиши (в своб"
...
async def answer_12_03(message: Message):
    s = message.text[:100]  # ограничиваем фантазию пользователя 100 символами
    await db_save_task(message.from_user.id, 12, s)
    await message.answer("Воссоздание «портрета» эмоции – это только один из способов. Еще можно управлять эмоциями чере"
                         "з дыхание, физические упражнения, и даже приятные запахи/вкусы/тактильные ощущения. Это все п"
                         "ереключает наше тело на нужную эмоцию. Используй эти способы.")
