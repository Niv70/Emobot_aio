...
async def answer_08_09(message: Message, state: FSMContext):
    ...
    if d == 1:
        ...
        await message.answer("{}, большинство людей ответили бы, что это причина <b><i>злости</i></b> (цифра 6)"
                             ".".format(name_user))
...
async def answer_08_11(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    name_user = data.get("name_user")
    await message.answer("Я тут примерил эти ценности на себя и понял, что в первом случаея бы очень злился, что рушатс"
                         "я семейные планы, а во втором, наоборот, радовался, что появилась возможность реализовать сво"
                         "й потенциал. Как по-разному мы можем вести себя в одних и тех же ситуациях 😊\nЗная причину, т"
                         "ы можешь предположить, какую эмоцию испытает человек, как он будет реагировать, и выбрать для"
                         " себя нужную стратегию поведения с ним. А еще важно понимать, что эмоции разные по интенсивно"
                         "сти, и сильные эмоции сразу не возникают, как правило, им предшествуют менее интенсивные.")
...
async def answer_08_12(message: Message, state: FSMContext):
    ...
    if s == "Показать пример":
        await message.answer("Например, сотрудник работает над задачей, требующей предельной внимательности, и уже сдел"
                             ...
                             "отдела. Какую эмоцию он испытывает? Верно, пока раздражение. Он просит коллег разговарива"
                             "ть тише, но через несколько минут о тишине никто не помнит. И тут интенсивность эмоции во"
                             "зрастает и раздражение перерастает в злость. Если коллеги по-прежнему не услышат его, и д"
                             "о гнева недалеко!")
