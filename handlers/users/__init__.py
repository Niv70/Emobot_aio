from . import help
from . import stop
from . import start
from . import start_mess
from . import menu_mess
from . import dbcreate
from . import statistic
from . import pool_mess
from . import task_mess
from . import task_mess03
from . import task_mess04
from . import task_mess05
from . import task_mess06
from . import task_mess07
from . import task_mess08
from . import task_mess09
from . import task_mess10
from . import task_mess11
from . import task_mess12
# После start_mess, но до poll и tasks д.б. прописан обработчик команды настройки
from . import echo

