...
async def answer_04_01(message: Message):
    ...
    if s == "ликование" or s == "восторг":
        ...
        await message.answer("Интересно! А еще <b><i>ликование</i></b> и <b><i>восторг</i></b>! Я тоже иногда испытываю"
                             " иногда такую эмоцию и тогда слышу в свой адрес «Ой, распушил усы»!")
    img = open("./IMG/День_04_2.jpg", "rb")
    await message.answer_photo(img)
    await message.answer("Напиши название эмоции, которую ты видишь на этой картинке.")
    await Task04.next()
...
async def answer_04_02(message: Message):
    ...
    await message.answer_photo(img)
    await message.answer("Напиши название эмоции, которую ты здесь видишь.")
    await Task04.next()


# Обработчик ввода 2го ответа (ЭМОЦИЯ) к задачке "на прокачку" 4-го дня
@dp.message_handler(state=Task04.Answer_04_03)
async def answer_04_03(message: Message):
    s = message.text.lower()
    s = s[0:100]  # ограничиваем фантазию пользователя 100 символами
    await db_save_task(message.from_user.id, 4, s)
    ...
    await message.answer_photo(img)
    await message.answer("Напиши название эмоции девушки.")
    await Task04.next()


# Обработчик ввода 4го ответа (ЭМОЦИЯ) к задачке "на прокачку" 4-го дня
...
async def answer_04_04(message: Message):
    ...
    await message.answer_photo(img)
    await message.answer("Напиши название эмоции мальчика слева от стола.")
    await Task04.next()


# Обработчик ввода 5го ответа (ЭМОЦИЯ) к задачке "на прокачку" 4-го дня
@dp.message_handler(state=Task04.Answer_04_05)
async def answer_04_05(message: Message):
    s = message.text.lower()
    s = s[0:100]  # ограничиваем фантазию пользователя 100 символами
    await db_save_task(message.from_user.id, 4, s)
    if s == "триумф":
...
# Обработчик ввода 6го ответа (ЭМОЦИЯ) к задачке "на прокачку" 4-го дня
...
async def answer_04_06(message: Message):
    ...
    await message.answer("Спасибо, что поделился своим мнением об эмоциях героев на фото. Это был интересный опыт. И я "
