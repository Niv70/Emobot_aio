...
async def answer_03_03(message: Message, state: FSMContext):
    ...
    if s == "Интересный факт о композиторе":
        await message.answer("Мы с тобой прослушали музыкальный фрагмент Войцеха Киляра к фильму «Дракула»."
...
async def answer_03_05(message: Message, state: FSMContext):
    ...
    name_user = data.get("name_user")
    if s == "Интересный факт о композиторе":
        await message.answer("Мы с тобой прослушали сонату Бетховена, известную как «Лунная». Однако сам Бетховен"
...
async def answer_03_07(message: Message, state: FSMContext):
    ...
    name_user = data.get("name_user")
    if s == "Интересный факт о композиторе":
        await message.answer("Мы с тобой прослушали произведение лучшего композитора в истории человечества по версии"
