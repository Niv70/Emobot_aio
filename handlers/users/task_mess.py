...
async def answer_02_05(message: Message, state: FSMContext):
    ...
    if s == "Интересный факт о картине":
        ...
        await message.answer("{0}, предлагаю кликнуть на служебное сообщение «Следующая картина» под строкой ввода "
                             "текста".format(name_user))
        return
...
async def answer_02_17(message: Message, state: FSMContext):
    ...
    if s == "Интересный факт о картине":
        ...
        await message.answer("{0}, предлагаю кликнуть на служебное сообщение «Выход из галереи» под строкой ввода "
                             "текста".format(name_user))
        return
